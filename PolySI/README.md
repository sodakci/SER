...
Usage: verifier audit [-hV] [--no-coalescing] [--no-pruning] [-t=<type>] <path>
  ...
  -t, --type=<type>     history type: DBCOP, TEXT, PRHIST
...
The primary input format is predicate history (`PRHIST`). The remaining
non-predicate formats are kept only for small compatibility tests and legacy
conversion workflows.

There are three kinds of histories this verifier can accept:

1. Predicate history format (`history.prhist.jsonl`)

    These histories are stored either as a directory containing
    `history.prhist.jsonl`, or as the file itself. `PRHIST` is the default
    history type, so `--type=prhist` is optional:

    ```
    $ java -Xmx8g -jar PolySI/build/libs/PolySI-1.0.0-SNAPSHOT.jar audit PolySIHistories/predicate_high_intensity/search_32_420_18_10000_0.5_hotspot/hist-00000
    ```
