...
public class SERVerifier<KeyType, ValueType> {
    ...
    @Getter
    @Setter
    private static boolean compareLegacyPredicateEdges = false;
    ...
    public boolean audit() {
        ...
        if (compareLegacyPredicateEdges) {
            profiler.startTick("SER_LEGACY_PREDICATE_COMPARE");
            var legacyPredicateGraph = new KnownGraph<>(history);
            injectPredicateEdgesSER(history, legacyPredicateGraph);
            profiler.endTick("SER_LEGACY_PREDICATE_COMPARE");
            System.err.printf(
                    "[SER] Legacy predicate-edge compare: PR_WR=%d, PR_RW=%d (not used by AR SAT solver)\n",
                    countEdgesOfType(legacyPredicateGraph.getKnownGraphA(), EdgeType.PR_WR),
                    countEdgesOfType(legacyPredicateGraph.getKnownGraphB(), EdgeType.PR_RW));
        }
        ...
        if (!accepted) {
            ...
            conflicts.getRight().forEach(c -> {
                var addEdges = ((Consumer<Collection<SEREdge<KeyType, ValueType>>>) s -> s.forEach(e -> {
                    txns.add(e.getFrom());
                    txns.add(e.getTo());
                }));
    ...
    /**
     * Legacy diagnostic predicate-edge derivation.
     *
     ...
     * only for explicit compare/debug runs and tests of the legacy derivation
     * layer.</p>
     */
    ...
    private static <KeyType, ValueType> void injectConservativePredicateCandidatesSER(
        ...
        for (var obs : observations) {
            ...
            for (var entry : unresolvedByKey.entrySet()) {
                ...
                boolean keyCanAffectObservation = resultKeys.contains(key)
                        || writesOnKey.stream().anyMatch(w -> matchesPredicate(w, predicateRead));
        ...
        if (conservativePrWr > 0 || conservativePrRw > 0) {
            System.err.printf(
                    "[SER] Recorded legacy conservative predicate candidates (ignored by AR SAT solver): PR_WR=%d, PR_RW=%d%n",
