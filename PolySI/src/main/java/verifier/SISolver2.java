    ...
    SISolver2(History<KeyType, ValueType> history,
        ...
        var impliesCNF = ((BiFunction<Lit, Collection<SIEdge<KeyType, ValueType>>, Lit>) (
                lit, edges) -> edges.stream()
                        .filter(e -> e.getType().equals(EdgeType.RW) || e.getType().equals(EdgeType.PR_RW))
        ...
        var edgesInB = Stream.concat(
                collectEdges.apply(precedenceGraph.getKnownGraphB().asGraph(), EdgeType.RW).stream(),
                collectEdges.apply(precedenceGraph.getKnownGraphB().asGraph(), EdgeType.PR_RW).stream())
                .collect(Collectors.toList());
        for (var e1 : edgesInA) {
